...
export default function ResendActivation() {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const res = await axios.post('/api/v1/usuarios/reenviar-activacion/', { email });
            setSuccess(res.data?.message || 'Si el correo está registrado y la cuenta no está activa, te enviaremos un nuevo enlace.');
        } catch (err) {
    ...
    return (
        <AuthShell
            ...
            subtitle="Si no recibiste el correo o expiró, te enviamos un nuevo enlace."
        >
            {success && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                    <span>{isLoading ? 'Enviando…' : success ? 'Enlace enviado' : 'Reenviar enlace'}</span>
